...
void Odometry::update()
{
    ...
    switch(mode)
    {
        case NONE:
            ...
            break;
        case TWOWHEEL:
            rx=ROBOT_R*((w[0]+w[1])/2.0f);
            ry=ROBOT_R*((w[0]+w[1])/2.0f);
            W=(ROBOT_R/(2.0f*ROBOT_L))*(w[1]-w[0]);
            break;
        case OMUNI2:
            rx=ROBOT_R*w[0];
            ry=ROBOT_R*w[1];
            ...
            break;
        case OMUNI3:
            rx=ROBOT_R*(w[0]*cosf(5.0f*PI/12)+w[1]*cosf(PI/4.0f)-w[2]*cosf(PI/12.0f));
            ry=ROBOT_R*(-w[0]*sinf(5.0f*PI/12)+w[1]*sinf(PI/4.0f)+w[2]*sinf(PI/12.0f));
            angular=(-ROBOT_R/(3.0f*ROBOT_L))*(w[0]+w[1]+w[2]);
            rc+=angular;
            break;
        case OMUNI4:
            rx=ROBOT_R*(w[3]-w[1]);
            ry=ROBOT_R*(w[0]-w[2]);
            angular=(ROBOT_R / (4.0f * ROBOT_L)) * (w[0] + w[1] + w[2] + w[3]);
            rc+=angular;
            break;
        case MECANUM4:
            float a=ROBOT_A/2.0f;
            float b=ROBOT_B/2.0f;

            rx=(ROBOT_R/4.0f)*(w[0]+w[1]+w[2]+w[3]);
            ry=(ROBOT_R/4.0f)*(w[0]-w[1]-w[2]+w[3]);
            angular=(ROBOT_R/(4.0f*(b+a)))*(w[1]-w[0]-w[2]+w[3]);
            rc+=angular;
            break;
    }
    //Posture calculation
